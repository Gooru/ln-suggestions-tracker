  ...
  private void processMessage(Message<JsonObject> message) {
    ...
    switch (op) {
      case Constants.Message.MSG_OP_SUGGESTIONS_ADD:
        ...
        break;
      case Constants.Message.MSG_OP_LIST_USER_SUGGESTIONS_FOR_COURSE:
        MessageProcessor.buildUserSuggestionsForCourseProcessor(vertx, message).process()
            .setHandler(event -> finishResponse(message, event));
        break;
      case Constants.Message.MSG_OP_LIST_USER_SUGGESTIONS_IN_CLASS:
        MessageProcessor.buildUserSuggestionsInClassProcessor(vertx, message).process()
            .setHandler(event -> finishResponse(message, event));
        break;
      case Constants.Message.MSG_OP_LIST_USER_SUGGESTIONS_FOR_COMPETENCY:
        MessageProcessor.buildUserSuggestionsForCompetencyProcessor(vertx, message).process()
            .setHandler(event -> finishResponse(message, event));
        break;
      case Constants.Message.MSG_OP_LIST_USER_SUGGESTIONS_IN_CA:
        MessageProcessor.buildUserSuggestionsForCAProcessor(vertx, message).process()
            .setHandler(event -> finishResponse(message, event));
        break;
      default:
  ...
  private void finishResponse(Message<JsonObject> message,
    ...
    if (asyncResult.succeeded()) {
      ...
      if (asyncResult.cause() instanceof HttpResponseWrapperException) {
        ...
        MessageResponseWrapperException exception = (MessageResponseWrapperException) asyncResult
            .cause();
