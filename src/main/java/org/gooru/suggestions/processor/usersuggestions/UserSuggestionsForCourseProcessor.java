...
public class UserSuggestionsForCourseProcessor implements MessageProcessor {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(UserSuggestionsForCourseProcessor.class);
    ...
    private void fetchUserSuggestionForCourse(UserSuggestionsForCourseCommand command) {
        try {
            List<SuggestionTrackerModel> models = userSuggestionsService.fetchSuggestionsForCourse(command);
