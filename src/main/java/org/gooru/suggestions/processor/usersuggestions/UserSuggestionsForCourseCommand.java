...
class UserSuggestionsForCourseCommand {
    ...
    public static UserSuggestionsForCourseCommand builder(JsonObject input) {
        UserSuggestionsForCourseCommand result = UserSuggestionsForCourseCommand.buildFromJsonObject(input);
        result.validate();
        return result;
    }
    ...
    private void validate() {
        if (courseId == null) {
            ...
            throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Invalid userId");
        }
    }
